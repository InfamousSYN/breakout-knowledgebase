...
!!! note "Permissions"
    The OOBE runs in the context of the local `defaultuser0` account, which has `NT Authority\SYSTEM` privileges.
...
1. Press **SHIFT+FUNCTION+F10** to open a `cmd.exe` process.

!!! note "Note"
    The `cmd.exe` will open in the foreground.
